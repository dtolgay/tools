...
# Note: DT addition
def VectorizedKernel(q):
    """
    Un-normalized cubic-spline kernel function, vectorized for arrays.

    Arguments:
        q - NumPy array or Pandas Series containing radii at which to evaluate the kernel,
            scaled to the kernel support radius (between 0 and 1)
    """
    # Initialize the result with zeros of the same shape as q
    ...
    # Apply conditions
    result[mask1] = 1 - 6 * q[mask1]**2 + 6 * q[mask1]**3
    result[mask2] = 2 * (1 - q[mask2]) ** 3
    
    # Values of q > 1 are already 0, as initialized
