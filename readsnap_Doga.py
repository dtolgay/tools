...
def readsnap(sdir,snum,ptype,
    ...
    print("I am inside function readsnap")
    ...
    # This is DogaTolgay's addition - DT
    file.close()
...
########################################################################################################################################################
    ...
    def read_one_file(file_number:int, 
                      ptype:int=ptype,
                      fname_base:str=fname_base, 
                      fname_ext:str=fname_ext, 
                      massarr=massarr,
                      flag_sfr=flag_sfr,
        ...
        print("I am inside function read_one_file")

        # file.close()
        fname = fname_base+'.'+str(file_number)+fname_ext
        if(fname_ext=='.hdf5'):
            file = h5py.File(fname,'r') # Open hdf5 snapshot file
        else:
            file = open(fname) # Open binary snapshot file
        ...
        # now do the actual reading
        if(npart[ptype]>0):
            nL = 0  # Setting this to zero as I will read all files parallely 
            nR=nL + npart[ptype]
            pos[nL:nR,:]=input_struct[bname+"Coordinates"]
            vel[nL:nR,:]=input_struct[bname+"Velocities"]
            ids[nL:nR]=input_struct[bname+"ParticleIDs"]
            ...
            if (ptype == 0):
                return pos, vel, mass, ids, ugas, rho, hsml, nume, numh, sfr, metal, nR

            if (ptype == 4):
                return pos, vel, mass, ids, metal, stellage, nR

            if (ptype == 5): 
                return pos, vel, mass, ids, bhmass, bhmdot, nR

            else:
                return pos, vel, mass, ids, nR

    print('ptype==0: ', ptype==0)

    if (ptype==0):
        print("I am inside if")            
        # pos, vel, mass, ids, ugas, rho, hsml, nume, numh, sfr, metal, nR, fname = read_one_file(file_number=0, pytpe=ptype)
        read_one_file(file_number=0, ptype=ptype)        
        print(fname=fname)

    if (ptype==4):            
        pos, vel, mass, ids, metal, stellage, nR, fname = read_one_file(file_number=0, ptype=ptype)

    if (ptype==5):            
        pos, vel, mass, ids, bhmass, bhmdot, nR, fname = read_one_file(file_number=0, ptype=ptype)

    else:
        pos, vel, mass, ids, nR = read_one_file(file_number=0, ptype=ptype)         
