...
def readsnap(sdir,snum,ptype,
    ...
    pos=np.zeros([npartTotal[ptype],3],dtype=np.float32)
    vel=np.copy(pos)
    ids=np.zeros([npartTotal[ptype]],dtype=int)
    mass=np.zeros([npartTotal[ptype]],dtype=np.float32)
    if (ptype==0):
        ugas=np.copy(mass)
        rho=np.copy(mass)
        hsml=np.copy(mass) 
    ...
    if (ptype == 0 or ptype == 4) and (flag_metals > 0):
        metal=np.zeros([npartTotal[ptype],flag_metals],dtype=np.float32)
