...
def readsnap(sdir,snum,ptype,
    ...
    pos=np.zeros([npartTotal[ptype],3],dtype=np.float16)
    vel=np.copy(pos)
    ids=np.zeros([npartTotal[ptype]],dtype=int)
    mass=np.zeros([npartTotal[ptype]],dtype=np.float16)
    if (ptype==0):
        ugas=np.copy(mass)
        rho=np.copy(mass)
        hsml=np.copy(mass) 
    ...
    if (ptype == 0 or ptype == 4) and (flag_metals > 0):
        # metal=np.zeros([npartTotal[ptype],flag_metals],dtype=np.float16)
        # TODO: dtolgay addition
        # Only allocate memory for the 0th column (1D array)
        metal_zero_column = np.zeros(npartTotal[ptype], dtype=np.float16)  # or np.uint8 if scaled
    ...
    for i_file in range(numfiles):
        ...
        if(npart[ptype]>0):
            ...
            if (ptype == 0 or ptype == 4) and (flag_metals > 0):
                ...
                # TODO: Change by dtolgay. 
                # metal[nL:nR,:]=metal_t
                metal_zero_column[nL:nR] = metal_t[:, 0]  # Extract first column                
            if (ptype == 4) and (flag_sfr > 0) and (flag_stellarage > 0):
